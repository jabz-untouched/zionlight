...
async function getContactData() {
  ...
  return { contactContent, siteSettings };
}

export default async function ContactPage() {
  const { contactContent, siteSettings } = await getContactData();
  
  // Extract content with fallbacks
  const heroContent = contactContent["hero"];
  
  // Build contact info from site settings
  const contactInfo = [
    {
      icon: (
      ...
      title: "Visit Us",
      content: siteSettings["contact-address"] || defaultContent.address,
    },
    {
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      title: "Email Us",
      content: siteSettings["contact-email"] || defaultContent.email,
    },
    {
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      title: "Call Us",
      content: siteSettings["contact-phone"] || defaultContent.phone,
    },
  ];
  
  // Build social links from site settings
  const socialLinks = [
    { key: "facebook", url: siteSettings["social-facebook"] },
    { key: "instagram", url: siteSettings["social-instagram"] },
    { key: "twitter", url: siteSettings["social-twitter"] },
  ].filter(s => s.url);
  return (
    <>
      {/* Hero Section */}
